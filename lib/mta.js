...
Mta.prototype.schedule = function (stopId, feedId) {
  ...
  var stopIds;
  var direction, obj;
  var options = _.pick(this.options, [ 'feed_id' ]);

  if (feedId) {
    options.feed_id = feedId;
  }

  var feedUrl = this.urls.gtfs + (options.feed_id ? '-' +options.feed_id : '');
  ...
  return fetch(feedUrl, { headers: { "x-api-key": this.options.key } })
  .then(parsers.gtfs)
  .then(function(data) {
